...
public sealed class ExecutorService : Executor.ExecutorBase
{
    ...
    public override async Task<GExecutorWrapper> Execute(GPlanOperator request, ServerCallContext context)
    {
        ...
        {
            throw new AsSQLExecuteException("Data source not found for the given plan operator.", ConnectorType.Postgres);
        }

        var planOperator = request.FromGrpcModel();
        if (planOperator is null)
        {
            throw new AsSQLExecuteException("Failed to convert the gRPC plan operator to the internal model.", ConnectorType.Postgres);
        }
