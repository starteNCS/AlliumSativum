...
public sealed class ReconstructionDistanceService
{
    ...
    public async Task<ReconstructionSimilarityResult> ReconstructionSimilarityOfDatasourceAsync(Guid datasourceId, List<string> ignoreAttributes)
    {
        ...
        {
            if (ignoreAttributes.Contains(attribute["attributename"]))
            {
                continue;
            }
            
            var query = $"SELECT x.{attribute["attributename"]} FROM {attribute["datasourcename"]}->{attribute["relationname"]} x";
            queries.Add(query);
        }
    ...
    private static double CalculateNormalizedEmd(Dictionary<double, int> original, Dictionary<double, double> reconstructed)
    {
        if (original.Count == 0 || reconstructed.Count == 0)
        {
            return 1.0;
        } 
