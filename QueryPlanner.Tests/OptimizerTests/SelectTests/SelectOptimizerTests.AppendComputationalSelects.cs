...
public sealed class AppendComputationalSelectsTests
{
    ...
    public void Should_Append_To_Only_SelectDto()
    {
        ...
        foreach (var attribute in hiddenAttributes)
            dto.Select.Should().ContainSingle(s =>
                s is AttributeSpecifier && ((AttributeSpecifier)s).Equals(attribute) &&
                ((AttributeSpecifier)s).IsHidden);
    }
    ...
    public void Should_Append_To_Correct_SelectDto()
    {
        ...
        foreach (var attribute in hiddenAttributes)
            ...
            {
                case "algorithm":
                    dtoAlgorithm.Select.Should().ContainSingle(s =>
                        s is AttributeSpecifier && ((AttributeSpecifier)s).Equals(attribute) &&
                        ((AttributeSpecifier)s).IsHidden);
                    break;
                case "experiment_run":
                    dtoExperimentRun.Select.Should().ContainSingle(s =>
                        s is AttributeSpecifier && ((AttributeSpecifier)s).Equals(attribute) &&
                        ((AttributeSpecifier)s).IsHidden);
                    break;
            }
