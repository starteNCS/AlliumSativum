    ...
    public static List<List<T>> GetPermutations<T>(this List<T> items)
    {
        if (items.Count > 5)
            throw new InvalidOperationException(
                "Generating permutations for more than 5 items is not supported due to performance reasons.");
        ...
        for (var i = 0; i < items.Count; i++)
        {
            ...
            foreach (var permutation in remaining.GetPermutations()) results.Add([current, ..permutation]);
        }
