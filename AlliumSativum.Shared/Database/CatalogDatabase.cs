...
public sealed class CatalogDatabase : IDisposable, IAsyncDisposable
{
    ...
    {
        if (_transaction != null) return await _txConnection!.ExecuteAsync(query, parameters, _transaction);
        await using var connection = new NpgsqlConnection(_connectionString);
        await connection.OpenAsync();
    ...
    public async Task<DataSourceEntity?> GetDataSourceAsync(Guid dataSource)
    {
        var dataSources = await QueryAsync<DataSourceEntity>(
            "SELECT * FROM Catalog.DataSources WHERE Id = @DataSourceId",
            new
            {
    ...
    {
        var relations = await QueryAsync<RelationEntity>(
            "SELECT * FROM Catalog.Relations WHERE Name LIKE @TableName AND DataSourceId = @DataSourceId",
            new
            {
                TableName = $"%{tableName}",
                DataSourceId = dataSource
    ...
    public async Task<List<RelationEntity>> GetRelationsOfDataSourceAsync(Guid dataSource)
    {
        var dataSources = await QueryAsync<RelationEntity>(
            "SELECT * FROM Catalog.Relations WHERE DataSourceId = @DataSourceId",
            new
            {
                DataSourceId = dataSource
            });
