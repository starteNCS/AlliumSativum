...
public sealed class PopLookupTable
{
    ...
    {
        foreach (var tableSpecifier in tableSpecifiers) _lookupTable.Add(tableSpecifier, planOperator);
    }

    public PlanOperator? GetAndRemove(TableSpecifier tableSpecifier)
    {
        if (!_lookupTable.TryGetValue(tableSpecifier, out var planOperator)) return null;
        ...
        foreach (var key in tablesToRemove) _lookupTable.Remove(key);
    ...
    public PlanOperator Single()
    {
        return _lookupTable.Values.Distinct().Single();
    }
}